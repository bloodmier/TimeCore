...
type Props = { data: Array<{ name: string; hours: number }> };

/**
 * HoursByUserChart
 *
 * Bar chart showing hours grouped by employee/user.
 * Uses ResizeObserver to calculate a responsive bar size and category gap.
 * The gradient id is unique to prevent SVG <defs> collisions if multiple charts are rendered.
 */
export function HoursByUserChart({ data }: Props) {
  ...
  // Compute dynamic barSize/gap so bars fit without horizontal scrolling.
  ...
  const barSize = Math.max(4, Math.min(Math.floor(slot * 0.7), 30));
      ...
      <div style={{ height: "18rem" }}>
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <defs>
              <linearGradient
                id={`barFillUser-${gradientId}`}
                x1="0"
                y1="0"
                x2="0"
                y2="1"
              >
                <stop offset="0%" stopColor="var(--chart-bar-hours-3)" />
                <stop offset="50%" stopColor="var(--chart-bar-hours)" />
                <stop offset="100%" stopColor="var(--chart-bar-hours-2)" />
              </linearGradient>
            </defs>
            ...
            >
              {chart.map((_, i) => (
                <Cell key={i} fill={`url(#barFillUser-${gradientId})`} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
